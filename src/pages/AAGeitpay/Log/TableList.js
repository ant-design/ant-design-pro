...
const CreateForm = Form.create()(props => {
  ...
  const array = `${record.fParam}`.split(',');

  const sqlArray = `${record.fSql}`.split('\n');
...
export default class TableList extends PureComponent {
  ...
  getColumns = table => {
    ...
    return [
      ...
      },
      {
        ...
        render: (text, record) => (
          <Fragment>
            <a onClick={() => this.handleModalUpdate(true, record, 1)}>查看</a>
          </Fragment>
        ),
      },
      {
  ...
  handleModalUpdate = (flag, tableRow, index) => {
    this.setState({
      modalVisible: !!flag,
      tableRow,
      addOrUpdate: 2,
