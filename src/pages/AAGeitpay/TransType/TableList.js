...
const CreateForm = Form.create()(props => {
  ...
  const { fTranscode, fTransname, fTranstype } = record;
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="交易码">
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="交易名称">
        {form.getFieldDecorator('fTransname', {
          initialValue: fTransname,
          rules: [{ required: true, message: '请输入交易名称' }, { max: 6, message: '不超过6位' }],
        })(<Input placeholder="请输入" />)}
      </FormItem>
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="交易类型">
        {form.getFieldDecorator('fTranstype', {
          initialValue: fTranstype,
          rules: [{ required: true, message: '请输入交易类型' }, { max: 4, message: '不超过4位' }],
        })(<Input placeholder="请输入" />)}
      </FormItem>
    </Modal>
  );
...
export default class TableList extends PureComponent {
  getColumns = table => {
    ...
    return [
      {
        title: '交易码',
        dataIndex: 'fTranscode',
      },
      {
        title: '交易名称',
        dataIndex: 'fTransname',
      },
      {
        title: '交易类型',
