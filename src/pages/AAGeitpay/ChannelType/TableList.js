...
const CreateForm = Form.create()(props => {
  ...
  const { fType, fName, fTerm } = record;
    ...
    >
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="类型代码">
        {form.getFieldDecorator('fType', {
          initialValue: fType,
          rules: [{ required: true, message: '请输入类型代码' }, { max: 2, message: '不超过2位' }],
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="默认终端号">
        {form.getFieldDecorator('fTerm', {
          initialValue: fTerm,
          rules: [
            { required: true, message: '请输入默认终端号' },
