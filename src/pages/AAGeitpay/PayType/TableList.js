...
const CreateForm = Form.create()(props => {
  ...
  const { fType, fThirdid, fName } = record;
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="支付类型代码">
        {form.getFieldDecorator('fType', {
          initialValue: fType,
          rules: [
            { required: true, message: '请输入支付类型代码' },
            { max: 4, message: '不超过4位' },
          ],
        })(<Input placeholder="请输入" />)}
      </FormItem>
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="支付类型名称">
        {form.getFieldDecorator('fName', {
          initialValue: fName,
          rules: [
            { required: true, message: '请输入支付类型名称' },
